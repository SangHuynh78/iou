...
#ifndef F_CPU
# warning "F_CPU not defined for TWI, auto defined 8M"
...
static    uint8_t twi_master_txBuffer[TWI_BUFFER_LENGTH];		//Master Transmitter buffer
static     volatile uint8_t twi_master_txBufferIndex;
static     volatile uint8_t twi_master_txBufferLength;

static		uint8_t twi_master_rxBuffer[TWI_BUFFER_LENGTH];		//Master Receiver buffer
static		volatile uint8_t twi_master_rxBufferIndex;
static		volatile uint8_t twi_master_rxBufferLength;

static    uint8_t twi_slv_txBuffer[TWI_BUFFER_LENGTH];			//Slave Transmitter buffer
static    volatile uint8_t twi_slv_txBufferIndex;
static    volatile uint8_t twi_slv_txBufferLength;

static    uint8_t twi_slv_rxBuffer[TWI_BUFFER_LENGTH];			//Slave Receiver buffer
static    volatile uint8_t twi_slv_rxBufferIndex;

static volatile uint8_t twi_error;

uint8_t* twi_get_master_tx_Buffer() {return twi_master_txBuffer;}
uint8_t* twi_get_master_rx_Buffer() {return twi_master_rxBuffer;}
...
void twi_init(void)
{
  ...
  //SCL CLK= TWI_FREQ = F_CPU/(16+2*TWBR*Prescaler)
  //TWPS0 =0, TWPS1 =0 => Prescaler = 1.
  cbi(TWSR, TWPS0);
  cbi(TWSR, TWPS1);
  
  //TWBR = ((F_CPU / TWI_FREQ) - 16) / 2;
  TWBR = 32; //TWI_FREQ =100K
 ...
 */
uint8_t twi_readFrom(uint8_t address, uint8_t length, uint8_t sendStop)
{
  uint8_t i;
  ...
  //check if twi ready, return 0 if not ready
  ...
  twi_master_rxBufferIndex = 0;
  twi_master_rxBufferLength = length-1;  // This is not intuitive, read on...
  ...
  if (twi_master_rxBufferIndex < length) {
    length = twi_master_rxBufferIndex;
  }
  // copy twi buffer to data
  //for(i = 0; i < length; ++i){
  //data[i] = twi_master_rxBuffer[i];
  //}
  return length;
}
...
uint8_t twi_writeTo(uint8_t address, uint8_t* data, uint8_t length, uint8_t wait, uint8_t sendStop)
{
  ...
  // initialize buffer iteration vars
  twi_master_txBufferIndex = 0;
  twi_master_txBufferLength = length;
  
  // copy data to twi buffer
  for(i = 0; i < length; ++i){
    twi_master_txBuffer[i] = data[i];
  }
  
  // build sla+w, slave device address + w bit
  twi_slarw = TW_WRITE;
  twi_slarw |= SLV_ADDRESS << 1;
...
uint8_t twi_transmit(const uint8_t* data, uint8_t length)
{
  ...
  // ensure data will fit into buffer
  if(TWI_BUFFER_LENGTH < (twi_slv_txBufferLength+length)){
    return 1;
  }
  ...
  for(i = 0; i < length; ++i){
    twi_slv_txBuffer[twi_slv_txBufferLength+i] = data[i];
  }
  twi_slv_txBufferLength += length;
...
ISR(TWI_vect)
{
  switch(TW_STATUS){
    ...
    case TW_REP_START: // sent repeated start condition
	  ...
	  //twi_slarw |= SLV_ADDRESS << 1;
      TWDR = twi_slarw;
      TWCR=(1<<TWEN)|(0<<TWSTA)|(1<<TWINT)|(1<<TWIE);
	  //twi_reply(1);
      break;
    ...
    case TW_MT_DATA_ACK: // slave receiver acked data
      ...
      if(twi_master_txBufferIndex < twi_master_txBufferLength){
        // copy data to output register and ack
        TWDR = twi_master_txBuffer[twi_master_txBufferIndex++];
        twi_reply(1);
      }else{
        if (twi_sendStop){
          twi_stop();
    ...
    case TW_MR_DATA_ACK: // data received, ack sent
      ...
      twi_master_rxBuffer[twi_master_rxBufferIndex++] = TWDR;
      //__attribute__ ((fallthrough));
    case TW_MR_SLA_ACK:  // address sent, ack received
      // ack if more bytes are expected, otherwise nack
      if(twi_master_rxBufferIndex < twi_master_rxBufferLength){
        twi_reply(1);
      }else{
        twi_reply(0);
      }
    ...
    case TW_SR_ARB_LOST_GCALL_ACK: // lost arbitration, returned ack
      ...
      twi_slv_rxBufferIndex = 0;
      twi_reply(1);
      break;
    case TW_SR_DATA_ACK:       // data received, returned ack
    case TW_SR_GCALL_DATA_ACK: // data received generally, returned ack
      // if there is still room in the rx buffer
      if(twi_slv_rxBufferIndex < TWI_BUFFER_LENGTH){
        // put byte in buffer and ack
        twi_slv_rxBuffer[twi_slv_rxBufferIndex++] = TWDR;
        twi_reply(1);
      }else{
        // otherwise nack
        twi_reply(0);
    ...
    case TW_SR_STOP: // stop or repeated start condition received
      ...
      if(twi_slv_rxBufferIndex < TWI_BUFFER_LENGTH){
        twi_slv_rxBuffer[twi_slv_rxBufferIndex] = '\0';
      }
      // callback to user defined callback
      twi_onSlaveReceive(twi_slv_rxBuffer, twi_slv_rxBufferIndex);
    ...
    case TW_ST_ARB_LOST_SLA_ACK: // arbitration lost, returned ack
      ...
      twi_slv_txBufferIndex = 0;
      // set tx buffer length to be zero, to verify if user changes it
      twi_slv_txBufferLength = 0;
      // request for txBuffer to be filled and length to be set
      // note: user must call twi_transmit(bytes, length) to do this
      twi_onSlaveTransmit();
      // if they didn't change buffer & length, initialize it
      if(0 == twi_slv_txBufferLength){
        twi_slv_txBufferLength = 1;
        twi_slv_txBuffer[0] = 0x00;
      }
      //__attribute__ ((fallthrough));		  
      // transmit first byte from buffer, fall
    case TW_ST_DATA_ACK: // byte sent, ack returned
      // copy data to output register
      TWDR = twi_slv_txBuffer[twi_slv_txBufferIndex++];
      // if there is more to send, ack, otherwise nack
      if(twi_slv_txBufferIndex < twi_slv_txBufferLength){
        twi_reply(1);
      }else{
        twi_reply(0);
      }

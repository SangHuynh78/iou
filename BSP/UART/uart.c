...
#define USART_RX_ERROR_MASK ((1 << FE) | (1 << DOR) | (1 << UPE)) // 
...
void usart0_close() {
    ...
    while(!(UCSR0A & (1 << TXC)));										// Wait for Tx unit to transmit the LAST character of ringbuffer
...
{
	uint8_t	udx;
	for(udx = 0;udx < len; udx++)	 usart1_send_char(*str++);
}
...
uint16_t usart1_read_char(void) {
	if (!rbuffer_empty(&p_UART1_meta->rb_rx)) {
		return (((p_UART1_meta->usart_error & USART_RX_ERROR_MASK) << 8) | (uint16_t)rbuffer_remove(&p_UART1_meta->rb_rx));
	}
	else {
		return (((p_UART1_meta->usart_error & USART_RX_ERROR_MASK) << 8) | USART_NO_DATA);     // Empty ringbuffer
	}
}

void usart1_close() {
	while(!rbuffer_empty(&p_UART1_meta->rb_tx));                        // Wait for Tx to transmit ALL characters in ringbuffer

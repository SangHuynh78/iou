...
{
	{0,	0, 0, 0},			//NTC_channel_temperature[4]
	{0,	0},					//onewire_channel_temperature[2]
	0,						//i2c_sensor_temperature
	{250, 250, 250, 250},	//channel_temperature_setpoint[4] set point is 25*C
	...
	{0,	0, 0, 0},			//DAC val
	0,						//DAC status
	{HEATING, HEATING, HEATING, HEATING}	//mode TEC
};
...
static	void	temperature_task_update(void)
{
	...
	s_Temperature_CurrentState.onewire_channel_temperature[0] = 0x7FFF /*get_onewire_temperature(0)*/;
	s_Temperature_CurrentState.onewire_channel_temperature[1] = 0x7FFF /*get_onewire_temperature(1)*/;
...
void	temperature_TEC_hysteris_control_cooling(uint16_t	NTC_temperature, uint8_t TEC_channel)
{
	...
	if (!(s_Temperature_CurrentState.TEC_status & (1<<(TEC_channel * 2))))		//TEC is disabling
	{
		...
		//  If temperature is lower than setpoint  but TEC is disabled
		else if (NTC_temperature <  (s_Temperature_CurrentState.channel_temperature_setpoint[TEC_channel] - TEMPERATURE_HYSTERIS))
		{
			// check if temperature is still increasing, add to the time count
			if ((NTC_temperature <  s_Temperature_CurrentState.NTC_channel_temperature[TEC_channel])  || (NTC_temperature <  (s_Temperature_CurrentState.channel_temperature_setpoint[TEC_channel] - TEMPERATURE_MAX_ERROR)))
			{
...
void	temperature_set_point(int16_t	setpoint, uint8_t channel)
{
...
void	temperature_enable_auto_control_TEC(uint8_t	channel)
{
	...
	temperature_set_TEC_output(channel , s_Temperature_CurrentState.mode[channel],s_Temperature_CurrentState.TEC_output_voltage[channel] );
}
...
void	temperature_get_status(void)
{
	...
	{
		if (s_Temperature_CurrentState.NTC_channel_temperature[i] == 0x7FFF) UARTprintf("NTC[%d] is fail\r\n", i);
		else UARTprintf("NTC[%d]:%i\r\n",i , s_Temperature_CurrentState.NTC_channel_temperature[i]);
	}
	if (s_Temperature_CurrentState.i2c_sensor_temperature == 0x7FFF) usart0_send_string("BMP390 is fail\r\n");
	else UARTprintf("BMP390 temp: %i \r\n", s_Temperature_CurrentState.i2c_sensor_temperature);

	//TECC
	uint8_t	_status = s_Temperature_CurrentState.TEC_status;
	UARTprintf("Setpoint: %i %i %i %i \r\nVout: %d %d %d %d \r\nAuto: %d %d %d %d \r\nEna: %d %d %d %d \r\n",
	...
	usart0_send_string("Mode: ");
	for (uint8_t idx = 0; idx <4; idx++)
	{
		if (s_Temperature_CurrentState.mode[idx] == 1) usart0_send_string("H  ");
		else usart0_send_string("C  ");
	}
	usart0_send_string("\r\n");
}
